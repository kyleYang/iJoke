...
- (AddVideoTaskStatus)addDownloadTaskForSaveVideo:(SaveVideo *)sVideo{
    ...
    if (M3U8InfoList != nil) {
        int taskId = [FTSNetwork videoM3u8Downloader:self.downloader Url:sVideo.content PkgFile:nil Target:self Sel:@selector(m3u8InitFinished:) Attached:sVideo];
        if (taskId <= 0) {
...
- (AddVideoTaskStatus)addDownloadTaskSVideo:(SaveVideo *)sVideo{
    ...
    M3U8SegmentInfoList *M3U8InfoList = [self segmentInfoListForVideoId:sVideo.youkuId];
    if (M3U8InfoList != nil) {
       int taskId = [FTSNetwork videoM3u8Downloader:self.downloader Url:sVideo.content PkgFile:nil Target:self Sel:@selector(m3u8DownloaderFinished:) Attached:sVideo];
        if (taskId<=0) {
            return TaskStatusFailed;
        }
        return TaskStatusSuccess;
...
- (AddVideoTaskStatus)addDownloadTaskForVideo:(Video *)video withStep:(VideoScreen)step{
    ...
    SaveVideo *sVideo = [[SaveVideo alloc] initWithVideo:video withStep:step];
