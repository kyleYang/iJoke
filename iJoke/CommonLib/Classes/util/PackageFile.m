...
+ (unsigned int) calcHasnforDataName:(NSString*)str;
...
+ (unsigned int) calcHasnforDataName:(NSString*)str {
    ...
    if(str == nil || [str length] < 1) return 1;
    ...
    unsigned int ret = [outputString hash];
...
-(BOOL)writeDataName:(NSString*)name Data:(NSData*)data {
    ...
    // update index
    item->nameHash = [PackageFile calcHasnforDataName:name];
    if(0 == item->size) {
...
//    t_DATA_ITEM *item = (t_DATA_ITEM*)[v pointerValue];
    t_DATA_ITEM *item = [self.mapDatas getPtrForKey:[PackageFile calcHasnforDataName:name]];
    if(NULL == item) {
        BqsLog(@"data not exist: %@", name);
        return nil;
    }
...
-(void)deleteDataName:(NSString*)name Flush:(BOOL)bFlush {
    ...
    NSUInteger key = [PackageFile calcHasnforDataName:name];
